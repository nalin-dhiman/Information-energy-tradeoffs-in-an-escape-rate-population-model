...
def evaluate_point_stage2(theta_dict, cfg, n_trials=20, seeds=[0,1,2], tau=0.02, cutoff=50.0):
    ...
    for seed in seeds:
        ...
        lcfg['seed'] = seed
        lcfg.setdefault('feature_mode', 'rate_lags')
        ...
        try:
            ...
            print(f"LOWER_ESTIMATOR_FAILED seed={seed}: {repr(e)}", flush=True)
            I_lower = np.nan

        # Diagnostic upper. Keep this separate so a spectral-estimator failure
        # cannot erase a valid decoding estimate.
        try:
            ...
            print(f"UPPER_ESTIMATOR_FAILED seed={seed}: {repr(e)}", flush=True)
            I_upper = np.nan
    ...
    return {
        'params': theta_dict,
        'I_lower_mean': vals['I_lower'].mean(skipna=True),
        'I_lower_std': vals['I_lower'].std(skipna=True),
        'I_upper_mean': vals['I_upper_surrogate'].mean(skipna=True),
        'I_upper_std': vals['I_upper_surrogate'].std(skipna=True),
